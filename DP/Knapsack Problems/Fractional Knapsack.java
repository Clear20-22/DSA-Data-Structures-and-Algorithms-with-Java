...
class Solution {
    static class tuple implements Comparable<tuple>{
        ...
        public int compareTo(tuple that){
            return Double.compare(that.perwt,this.perwt);
        }
    ...
    double fractionalKnapsack(int[] values, int[] weights, int W) {
        // code here
        List<tuple> store = new ArrayList<>();
        
        for(int i=0;i<values.length;i++){
            store.add(new tuple((double)values[i],(double)weights[i]));
        }
        ...
        for(int i=0;i<store.size();i++){
            if(W > 0){
                if(W >= store.get(i).wt){
                    ...
                    ans += store.get(i).perwt * (double)W;
                    W = 0;
                }
            } else {
                return ans;
